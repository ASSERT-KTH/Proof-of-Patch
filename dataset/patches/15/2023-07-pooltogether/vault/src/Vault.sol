...
contract Vault is ERC4626, ERC20Permit, ILiquidationSource, Ownable {
     ...
     * @param reason The revert reason that was thrown
     */
    event ClaimFailed(address indexed winner, uint8 indexed tier, uint32 indexed prizeIndex, bytes reason);
     ...
     */
    function depositWithPermit(uint256 _assets, address _receiver, uint256 _deadline, uint8 _v, bytes32 _r, bytes32 _s)
        external
        returns (uint256)
    {
     ...
     */
    function mintWithPermit(uint256 _shares, address _receiver, uint256 _deadline, uint8 _v, bytes32 _r, bytes32 _s)
        external
        returns (uint256)
    {
        uint256 _assets = _beforeMint(_shares, _receiver);
     ...
     * @return uint256 Amount of shares minted to `_receiver`.
     */
    function sponsorWithPermit(uint256 _assets, address _receiver, uint256 _deadline, uint8 _v, bytes32 _r, bytes32 _s)
        external
        returns (uint256)
    {
        _permit(IERC20Permit(asset()), msg.sender, address(this), _assets, _deadline, _v, _r, _s);
    ...
    /// @inheritdoc ERC4626
    function withdraw(uint256 _assets, address _receiver, address _owner) public virtual override returns (uint256) {
    ...
    /// @inheritdoc ERC4626
    function redeem(uint256 _shares, address _receiver, address _owner) public virtual override returns (uint256) {
        ...
        for (uint256 w = 0; w < _winners.length; w++) {
            ...
            for (uint256 p = 0; p < prizeIndicesLength; p++) {
                try this.claimPrize_INTERNAL_USE_ONLY(
                    _winners[w], _tier, _prizeIndices[w][p], _feePerClaim, _feeRecipient
                ) returns (uint256 prize) {
                    totalPrizes += prize;
                } catch (bytes memory reason) {
                    emit ClaimFailed(_winners[w], _tier, _prizeIndices[w][p], reason);
                }
            }
        }
     ...
     * @return uint256 Amount of assets represented by the shares
     */
    function _convertToAssets(uint256 _shares, uint256 _exchangeRate, Math.Rounding _rounding)
        internal
        view
        returns (uint256)
    {
    ...
    function claimPrize_INTERNAL_USE_ONLY(
        ...
        if (hooks.useBeforeClaimPrize) {
            try hooks.implementation.beforeClaimPrize{gas: HOOK_GAS}(_winner, _tier, _prizeIndex, _fee, _feeRecipient)
        ...
        uint256 prizeTotal = _prizePool.claimPrize(_winner, _tier, _prizeIndex, recipient, _fee, _feeRecipient);

        if (hooks.useAfterClaimPrize) {
            try hooks.implementation.afterClaimPrize{gas: HOOK_GAS}(
                _winner, _tier, _prizeIndex, prizeTotal - _fee, recipient
            ) {} catch (bytes memory reason) {
    ...
    function _currentExchangeRate() internal view returns (uint256) {
        ...
        uint256 _totalSupplyToAssets =
            _convertToAssets(_totalSupplyAmount, _lastRecordedExchangeRate, Math.Rounding.Down);
